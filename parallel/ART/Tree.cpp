...
namespace ART_unsynchronized {
    ...
    void findBoundary(N *node,const Key &k,uint32_t level,N *&first,N *&second){
     ...
     //查找的代码 单线程下的查找代码
     N* Tree::LookupLT(const Key &k) const{
        N *first = nullptr;
        N *second = nullptr;
        ...
        while (true) {
            ...
            switch (checkPrefix(node, k, level)) { // increases level
                ...
                case CheckPrefixResult::Match:
                    ...
                    if (nextNode->getType() == NTypes::Nleaf) {  //目前还没有增加边界条件的判断
                        
                        auto n1 = static_cast<LeafNode *>(nextNode);
                        TID tid = n1->getLeafValue();
    ...
    //         if (nextNode == nullptr) {
    //             //cout<<"insert 1"<<endl;
    //             //N *leaf = new LeafNode(tid,0,0);
    //             // N::setLeaf(tid)

    //             //const uint8_t *c = "a";

    //             //N* leaf = new N4(node->getPrefix(),12);
    //             auto leaf = new LeafNode(tid);
    //             N::insertA(node, parentNode, parentKey, nodeKey, leaf);
    //             return;
    //         }
        ...
        while (true) {
            ...
            switch (checkPrefixPessimistic(node, k, nextLevel, nonMatchingKey, remainingPrefix,
                ...
                case CheckPrefixPessimisticResult::NoMatch: {  
                    ...
                    newNode->insert(k[nextLevel], leafnode);
                    newNode->insert(nonMatchingKey, node);
            ...
            if (nextNode == nullptr) {
                //cout<<"insert 1"<<endl;
                auto leafnode = new LeafNode(tid);
                // if(leafnode ->getType() == NTypes::Nleaf){
                //     printf("True\n");
                // }
                // if(leafnode ->getType() == NTypes::N4){
                //     printf("False\n");
                // }
                //cout<<leafnode->getLeafValue()<<endl;
                N::insertA(node, parentNode, parentKey, nodeKey, leafnode);
                return;
            }
            if (nextNode->getType() == NTypes::Nleaf) {//那个位置是叶子节点 情况1：由1个节点变成2个节点可以执行到这里
                //cout<<"leaf"<<endl;
                Key key;
                //loadKey(N::getLeaf(nextNode), key);//因为实验的value和key一样，根据value 得到key
                ...
                auto n4 = new N4(&k[level], prefixLength);//为了区分2个节点需要建立内部节点
                auto leafnode = new LeafNode(tid); 
                n4->insert(k[level + prefixLength], leafnode);
                n4->insert(key[level + prefixLength], nextNode);
                N::change(node, k[level - 1], n4);//把这个n4节点插入到跟节点（N256节点）
