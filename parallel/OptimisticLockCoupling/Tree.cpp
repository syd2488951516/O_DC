...
namespace ART_OLC {
                ...
                case NTypes::N4: {
                    ...
                    if(node->keys[0] == keyByte ){//等于左边界的情况
                        ...
                        if(ss){
                            ...
                            if(nextNode->getType() == NTypes::Nleaf){
                                // add node 节点解锁
                                node->readUnlockOrRestart(v, needRestart);//在访问下一个节点的时候，需要验证父亲节点
                                if (needRestart) return NULL;
                                return nextNode;
    ...
    // }
      struct lessForData : public std::binary_function<cracker_column, int, bool> {
            bool operator() (cracker_column a, int b) const {return (a.first < b);}
      };
    ...
    int crackInTwoItemWise(IndexEntry *&c,  uint64_t posL, uint64_t posH, uint64_t med) {
        uint64_t x1 = posL, x2 = posH;
        while (x1 <= x2) {
            if (c[x1] < med)
                x1++;
            else {
      ...
      TID Tree::crack(IndexEntry *&c,uint64_t value,N *first, N *second, uint64_t limit, 
                                          ThreadInfo &threadInfo, bool &needRestart){
          uint64_t L,R;
          if (first && second) {
                auto n1 = static_cast<LeafNode *>(first);
                auto n2= static_cast<LeafNode *>(second);
                L = n1->getLeafValue();
                R = n2->getLeafValue()-1;
                //uint64_t nv = first->readLockOrRestart(needRestart);
                first->writeLockOrRestart(needRestart);//加写锁
                if (needRestart) return 0;

                //int position = partition(cracker_map + L, cracker_map + R, std::bind2nd(lessForData(), value)) - cracker_map;
                int position = crackInTwoItemWise(c,L,R,value);
                first->writeUnlock();
          } else if (first) {
                auto n1 = static_cast<LeafNode *>(first);
                L = n1->getLeafValue();
                R = limit;
                //cout<<first->typeVersionLockObsolete<<endl;
                first->writeLockOrRestart(needRestart);//加写锁
                //cout<<first->typeVersionLockObsolete<<endl;
                if (needRestart) return 0;
                ...
                auto n2= static_cast<LeafNode *>(second);
                R = n2->getLeafValue()-1;
                //R = ART_OLC::N::getLeaf(second)-1;
                second->writeLockOrRestart(needRestart);//加写锁
                if (needRestart) return 0;
    ...
    void printInfo(N * first,N *second){
         if (first && second) {
                auto n1 = static_cast<LeafNode *>(first);
                auto n2= static_cast<LeafNode *>(second);
                cout<<n1->getLeafValue()<<" "<< n2->getLeafValue()<<endl;
             
          } else if (first) {
                auto n1 = static_cast<LeafNode *>(first);
                cout<<n1->getLeafValue()<<" "<< 0 <<endl;
          } else if (second) {
                auto n2= static_cast<LeafNode *>(second);
                cout<<0<<" "<<n2->getLeafValue()<<endl;
          } 
    ...
    //查找的代码 多线程下的查找代码（多参考一下并发insert 和 并发 Look）
     N* Tree::LookupLT(const Key &k, N *&first,N *&second,ThreadInfo &threadInfo, 
                              uint64_t kk, IndexEntry *&c,uint64_t limit,uint64_t &position) const{
        //cout<<"sdsd"<<endl;
        //cout<<"kk = "<<kk<<endl;
        ...
        while (true) {
            ...
            switch (checkPrefix(node, k, level)) { // increases level
                case CheckPrefixResult::NoMatch://前缀不匹配的情况 //并发还没有写完
                    ...
                    //做分裂 + 写锁
                    position = crack(c,kk,first,second,limit,threadInfo,needRestart);
                    if (needRestart) goto restart;
                ...
                case CheckPrefixResult::Match:
                    ...
                    if (node == nullptr) {    //并发没有写完
                        ...
                        //cout<<"2 "<<kk<<" ";
                        //做分裂 + 写锁
                        if(!first && !second){// 第一次的时候，需要对父亲节点上锁
                            //cout<<"0"<<" "<<"0"<<endl;
                            parentNode->writeLockOrRestart(needRestart);
                            if (needRestart) goto restart;
                            //cout<<parentNode->typeVersionLockObsolete<<endl;
                            uint64_t L = 0 ,R = limit;
                            position = crackInTwoItemWise(c,L,R,kk);
        ...
        while (true) {
            ...
            switch (res) { // 在不匹配的情况下 需要新增内部节点
                ...
                case CheckPrefixPessimisticResult::NoMatch: {
                    ...
                    auto leaf1 = new LeafNode(tid);
                    newNode->insert(k[nextLevel],leaf1);
            ...
            if (nextNode == nullptr) {
                auto leafnode = new LeafNode(tid);
                //cout<<leafnode->getLeafValue()<<endl;
                N::insertAndUnlock(node, v, parentNode, parentVersion, parentKey, nodeKey,leafnode, needRestart, epocheInfo);
                if (needRestart) goto restart;
                return;
            }
            ...
            //查找到的是一个叶子节点
            if (nextNode->getType() == NTypes::Nleaf) {// 这个怎么都没有执行阿
                //cout<<"leaf"<<endl;
                //将当前节点的读锁升级为写锁
                node->upgradeToWriteLockOrRestart(v, needRestart);
                if (needRestart) goto restart;
                ...
                auto leaf3 = new LeafNode(tid);
                n4->insert(k[level + prefixLength], leaf3);
