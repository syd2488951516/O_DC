...
namespace ART_OLC {
    ...
    //update 20-12-25 21::51
    template<class NODE>
    void N16::copyTo(NODE *n) const {
        for (unsigned i = 0; i < count; i++) {
            n->insert(flipSign(keys[i]), children[i]);
            if(children[i]->getType() != NTypes::Nleaf){
                children[i]->parent = n;//当内部节点扩展的时候 node16的孩子 add parent
            }
            // if(!isLeaf(this->children[i])){
            //     this->children[i]->parent = n;//当内部节点扩展的时候 node16的孩子 add parent
            // }
        }
        n->parent = this->parent; 
        //std::cout<<"keys[0] = "<<+(keys[0])<<" keys[count-1]= "<<+(keys[count-1])<<std::endl;
        n->minl = flipSign(keys[0]);
        n->maxr = flipSign(keys[count-1]);
    }
    ...
    N *N16::getMinChild() const{// add function
        return children[0];
    }

    N *N16::getMaxChild() const{// add function
