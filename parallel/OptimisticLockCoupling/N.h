...
using cracker_column = std::pair<uint64_t,uint64_t>;
 ...
 */
    ...
    public:
        ...
        N(NTypes type){ //叶子节点用的构造函数
            // if(type == NTypes::Nleaf){
            //     printf("is leaf\n");
            // }
            setType(type);
        }
    ...
    public:
        ...
        //static N *getParent(N * node);
        template<typename curN>
        static N* getParent(curN* node);

        static N* minimum(N* node);

        static N* maximum(N* node);

        static N *getMinChild(N *node);//add get next layer

        static N *getMaxChild(N *node);//add get next layer
    ...
    public:
        ...
        N *getMinChild() const ;//add new function
        N *getMaxChild() const ;//add new function
   ...
   return n-(x&1);
    ...
    public:
     ...
     //add real leaf node
    class LeafNode : public N {
    public:
        uint64_t value ;

    public:
        //叶子节点的构造函数
        LeafNode(uint64_t val) : N(NTypes::Nleaf) {
            value = val;
         }

        // LeafNode(const uint8_t *prefix, uint32_t prefixLength) : N(NTypes::Nleaf, prefix,
        //                                                                        prefixLength) {
             
        // }

        void setLeafValue( uint64_t val){
            //std::cout<<&value<<std::endl;
            value = val;
        }
        
        uint64_t getLeafValue(){
            return value;
        }
